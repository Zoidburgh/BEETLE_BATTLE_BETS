...
class NetworkManager:
    ...
    def send_game_options(self, referee_enabled, ball_active, donut_mode=False, x_stage_mode=False, barbell_mode=False, yinyang_mode=False, hourglass_mode=False, tornado_mode=False, sandstorm_mode=False, ufo_mode=False, ice_mode=False, figure8_mode=False, squiggle_mode=False, hole_mode=False, comet_mode=False, square_mode=False, board_break_mode=False, conveyor_mode=False):
        """
        Host sends game options to guest.
        Packet format: [type:1][referee:1][ball:1][donut:1][x_stage:1][barbell:1][yinyang:1][hourglass:1][tornado:1][sandstorm:1][ufo:1][ice:1][figure8:1][squiggle:1][hole:1][comet:1][square:1][board_break:1][conveyor:1] = 19 bytes
        """
        if not self.is_host or not self.connected:
            return
        data = struct.pack('>BBBBBBBBBBBBBBBBBBB', MSG_GAME_OPTIONS,
    ...
    def _handle_packet(self, data, sender_id, input_buffer):
        ...
        if msg_type == MSG_INPUT:
            ...
            if len(data) >= 19 and not self.is_host:
                # New format with conveyor
                _, referee_enabled, ball_active, donut_mode, x_stage_mode, barbell_mode, yinyang_mode, hourglass_mode, tornado_mode, sandstorm_mode, ufo_mode, ice_mode, figure8_mode, squiggle_mode, hole_mode, comet_mode, square_mode, board_break_mode, conveyor_mode = struct.unpack('>BBBBBBBBBBBBBBBBBBB', data[:19])
                ...
                # Backwards compatibility with 17-byte format (no board_break, no conveyor)
                ...
                # Backwards compatibility with 16-byte format (no square, no board_break)
