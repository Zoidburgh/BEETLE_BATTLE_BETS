...
class NetworkManager:
    ...
    def send_game_options(self, referee_enabled, ball_active, donut_mode=False, x_stage_mode=False):
        """
        Host sends game options to guest.
        Packet format: [type:1][referee:1][ball:1][donut:1][x_stage:1] = 5 bytes
        """
        if not self.is_host or not self.connected:
            return
        data = struct.pack('>BBBBB', MSG_GAME_OPTIONS,
    ...
    def _handle_packet(self, data, sender_id, input_buffer):
        ...
        if msg_type == MSG_INPUT:
            ...
            if len(data) >= 5 and not self.is_host:
                ...
                # Backwards compatibility with old 4-byte format (no x_stage)
